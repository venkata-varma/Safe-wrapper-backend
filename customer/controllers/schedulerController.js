  ...
  schedule.scheduleJob(job_each_minute, async ()=> {
    ...
    // if(currentDateAndTime.getDay() === 0){
      ...
      for(let account of getAllActiveAccounts){
        await schedulerEmailJobs(await integrationsMasterModel.find({accountId:new mongoose.Types.ObjectId('667d4177bc77277e43bc1e2f'), status:"active"}), currentDateAndTime, accountLogo = account?.logo)
      }
    // }
  });
});
