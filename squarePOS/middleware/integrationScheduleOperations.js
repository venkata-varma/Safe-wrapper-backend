...
const initiateCronJob = async (integration) => {
    ...
    //-------------------------
    let createIntegrationsCron = await squarePOSIntegrationsCronsModel.create({
    ...
    await squarePOSIntegrationsSettingsModel.findOneAndUpdate({ accountId }, { $set: { lastPullDate: new Date(), lastIntgerationsCronId: createIntegrationsCron._id } }, { new: true, runValidators: true })
    ...
    await squarePOSIntegrationsCronsModel.findByIdAndUpdate(createIntegrationsCron._id,
        {
            $set: {
